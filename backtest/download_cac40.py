...
def download_and_process(ticker):
    try:
        # Fetching 2 years for warmup
        df = yf.Ticker(ticker).history(period="2y", auto_adjust=False, actions=False)
...
if __name__ == "__main__":
    print("Fetching CAC 40 list...")
    headers = {'User-Agent': 'Mozilla/5.0'}
    response = requests.get('https://en.wikipedia.org/wiki/CAC_40', headers=headers)
    cac40 = pd.read_html(io.StringIO(response.text))[4]
    tickers = cac40['Ticker'].tolist()

    print(f"Found {len(tickers)} tickers. Starting 2-year download for warmup...")
    ...
    print(f"CAC 40 data cached to {cache_path}")
